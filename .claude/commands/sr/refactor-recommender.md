...
Find deprecated APIs and old language syntax: `var` instead of `let`/`const`, callbacks instead of `async`/`await` where appropriate, legacy patterns with documented replacements, etc. Modernisation to current idioms is the expected refactoring.
...
---
_Generated by `/sr:refactor-recommender` in specrails-hub_
```
